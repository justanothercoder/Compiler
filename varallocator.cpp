...
VarAllocator::VarAllocator() : space_for_variables(0), space_for_locals(0), space_for_return_value(0) { }
...
{ 
	return var_addresses.at(var) + (!var->isParam() ? (space_for_locals + space_for_return_value) : 0);  
}

int VarAllocator::getAddress(ExprNode *expr) const { return locals.at(expr) + space_for_return_value; }

int VarAllocator::getAddressForLocal() const { return GlobalConfig::int_size; }

int VarAllocator::getSpace() const { return space_for_variables + space_for_locals + space_for_return_value; }
